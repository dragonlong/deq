...
# (1) interactive, (2) bash getMySQLprompt_MOVES3.sh;

# This script is used to open a MariaDB terminal to get inside MariaDB and get a database prompt
# Author:   Brian Marshall - ARC/Virginia Tech
# modified: Xiaolong Li of ARC/Virginia Tech, lxiaol9@vt.edu

#************************* Step 1. request for an interactive job: begin user inputs (do it on Linux prompt) *******************************
#salloc --partition=dev_q --time=2:00:00 --nodes=1 --tasks-per-node=12 -A deqtest

#************************* Step 2. load modules & set all needed paths *******************************
module purge
module load cmake/3.17.2 gcc jdk/1.11.0.9
export PATH=/groups/deq/cascades/apps/mariadb/10.4.12/bin:$PATH
export PATH=/groups/deq/cascades/models/MOVES3/statics_moves3_xiaolong/MOVES3_src/ant/bin:$PATH

# Note: Please put all the paths needed here, like mysql database, or MOVES3 src files, etc.

#existingInputDatabaseDir=/groups/deq/cascades/models/MOVES3/4-testruns/onroad_chesterfield_5tests/test5/mysql_out/outputs
#existingInputDatabaseDir=/groups/deq/cascades/models/MOVES3/4-testruns/onroad_chesterfield_5tests/Sonya_outfile/out

#existingInputDatabaseDir=/groups/deq/cascades/models/MOVES3/4-testruns/nonroad_chesterfield_2tests/test1/mysql_out/outputs
#existingInputDatabaseDir=/groups/deq/cascades/models/MOVES3/4-testruns/nonroad_chesterfield_2tests/test2/mysql_out/outputs

# existingInputDatabaseDir=/groups/deq/cascades/models/MOVES3/4-testruns/nonroad_chesterfield_2tests/Sonya_outfiles/out
existingInputDatabaseDir=/groups/deq/cascades/models/MOVES3/4-testruns/nonroad_chesterfield_2tests/test2_xiaolong/mysql_out/outputs
...
cp -r $movesSetupDir/databases/movesdb20201105  $runDir/mysql/data
...
#open mysql terminal, must with user=root option, else movesdb20201105
# does not show up in "show databases;" command
